...
"""
from day2.day2_part1 import read_data


def find_final_coordinates_with_aim(instructions: list[str]) -> tuple[int, int]:
    ...
    for instruction_string in instructions:
        ...
        if instruction == "forward":
            ...
            y_position += int(amount) * aim
        else:
            if instruction == "down":
                aim += int(amount)  # Note that
            else:
                aim -= int(amount)
...
if __name__ == "__main__":
    data = read_data()
    x_pos, y_pos = find_final_coordinates_with_aim(data)
