"""Day 3, part 1. Implementation in numpy.
...
def binary_to_decimal(binary_number: str) -> int:
    ...
    """
    bin_values = np.power(2, np.arange(len(binary_number)))[::-1]
    multiplied_values = np.array(list(binary_number)).astype(int) * bin_values
...
def index_vertically(input_data: np.ndarray) -> np.ndarray:
    ...
    :return: The vertically indexed data with size M x N.
    """
...
def invert_integers(integers: np.ndarray) -> np.ndarray:
    """Invert integers.
    ...
    """
    return np.abs(integers - 1)


if __name__ == "__main__":
    data = read_data()
    vertical_data = index_vertically(np.array(data))
